...
duplicateHelper [] l = l 
duplicateHelper xs l = duplicateHelper (init xs) l ++ [last xs, last xs] 
duplicate xs = duplicateHelper xs []
...
{-
 ...
 -}
ziphelper :: [a] -> [b] -> [(a, b)] -> [(a, b)]
ziphelper xs ys l = if null xs || null ys then l else ziphelper (tail xs) (tail ys) (l ++ [(head xs, head ys)])

ziplike :: [a] -> [b] -> [(a, b)]
ziplike xs ys = ziphelper xs ys []
...
rotate n l = if n == 0 then l else rotate (n - 1) (drop 1 l ++ [head l])
